...
This repo packages [Maple Proxy](https://github.com/OpenSecretCloud/maple-proxy) for StartOS.
...
Maple Proxy ships as two images in `startos/manifest/index.ts`: the upstream `maple-proxy` (pulled by `dockerTag`) and a locally built `maple-ui` (from `assets/ui/`). Upstream version bumps only move the proxy tag.

1. Bump the `maple-proxy` `dockerTag` in `startos/manifest/index.ts` to `ghcr.io/opensecretcloud/maple-proxy:<new version>`.
